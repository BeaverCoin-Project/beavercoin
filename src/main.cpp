...
#if defined(NDEBUG)
# error "BeaverCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BeaverCoin Signed Message:\n";
...
{
    // BeaverCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // BeaverCoin: 210k blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // BeaverCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // BeaverCoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BeaverCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BeaverCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Reject block.nVersion=1 blocks (mainnet >= 310000, testnet >= 400000)
        if (nVersion < 2)
        {
            if ((!fTestNet && nHeight >= 310000) ||
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // BeaverCoin: increase each by adding 4 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BeaverCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BeaverCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BeaverCoinMiner(CWallet *pwallet)
{
    printf("BeaverCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("beavercoin-miner");
    ...
    try { loop {
        ...
        printf("Running BeaverCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BeaverCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BeaverCoinMiner, pwallet));
}
